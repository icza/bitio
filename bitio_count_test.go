...
func TestCountReader(t *testing.T) {
	...
	eq(int64(0), r.GetBitPosition())

	expEq(byte(3))(r.ReadByte())
	eq(int64(8), r.GetBitPosition())

	expEq(uint64(255))(r.ReadBits(8))
	eq(int64(16), r.GetBitPosition())

	expEq(uint64(0xc))(r.ReadBits(4))
	eq(int64(20), r.GetBitPosition())

	expEq(uint64(0xc1))(r.ReadBits(8))
	eq(int64(28), r.GetBitPosition())

	expEq(uint64(0xabcde))(r.ReadBits(20))
	eq(int64(48), r.GetBitPosition())

	expEq(true)(r.ReadBool())
	eq(int64(49), r.GetBitPosition())

	expEq(false)(r.ReadBool())
	eq(int64(50), r.GetBitPosition())

	eq(uint8(6), r.Align())
	eq(int64(56), r.GetBitPosition())

	s := make([]byte, 2)
	expEq(2)(r.Read(s))
	eq(int64(72), r.GetBitPosition())

	eq(true, bytes.Equal(s, []byte{0x01, 0x02}))

	expEq(uint64(0xf))(r.ReadBits(4))
	eq(int64(76), r.GetBitPosition())

	expEq(2)(r.Read(s))
	eq(int64(92), r.GetBitPosition())
...
func TestCountWriter(t *testing.T) {
	for i := 0; i < 2; i++ {
		...
		eq(int64(0), w.GetBufferBitSize())

		eq(nil, w.WriteByte(0xc1))
		eq(int64(8), w.GetBufferBitSize())
		eq(nil, w.WriteBool(false))
		eq(int64(9), w.GetBufferBitSize())
		eq(nil, w.WriteBits(0x3f, 6))
		eq(int64(15), w.GetBufferBitSize())
		eq(nil, w.WriteBool(true))
		eq(int64(16), w.GetBufferBitSize())
		eq(nil, w.WriteByte(0xac))
		eq(int64(24), w.GetBufferBitSize())
		eq(nil, w.WriteBits(0x01, 1))
		eq(int64(25), w.GetBufferBitSize())
		eq(nil, w.WriteBits(0x1248f, 20))
		eq(int64(45), w.GetBufferBitSize())
		expEq(uint8(3))(w.Align())
		eq(int64(48), w.GetBufferBitSize())
		expEq(2)(w.Write([]byte{0x01, 0x02}))
		eq(int64(64), w.GetBufferBitSize())
		eq(nil, w.WriteBits(0x0f, 4))
		eq(int64(68), w.GetBufferBitSize())
		expEq(2)(w.Write([]byte{0x80, 0x8f}))
		eq(int64(84), w.GetBufferBitSize())
		expEq(uint8(4))(w.Align())
		eq(int64(88), w.GetBufferBitSize())
		expEq(uint8(0))(w.Align())
		eq(int64(88), w.GetBufferBitSize())
		eq(nil, w.WriteBits(0x01, 1))
		eq(int64(89), w.GetBufferBitSize())
		eq(nil, w.WriteByte(0xff))
		eq(int64(97), w.GetBufferBitSize())
		eq(uint8(7), w.TryAlign())
		eq(int64(104), w.GetBufferBitSize())
		w.WriteBitsUnsafe(0x1234, 16)
		eq(int64(120), w.GetBufferBitSize())
		eq(nil, w.Close())
...
func TestCountedChain(t *testing.T) {
	...
	eq(r.GetBitPosition()+int64(skipped), w.GetBufferBitSize())
}
